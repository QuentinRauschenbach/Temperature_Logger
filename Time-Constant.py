...
for i, file in enumerate(files[:]):
    ...
    plt.scatter(shock_time,tsensors["temperature"].values[shock_index],marker="x",c=colors[i],zorder=1)
    plt.scatter(end_time,Temp_end,marker="o",c=colors[i],zorder=2)
    plt.scatter(time_tau,Temp_tau,marker="o",c=colors[i],zorder=4)
    
    plt.plot(tsensors["millis"].values[start_index:end_index],tsensors["temperature"].values[start_index:end_index], label=file.split("/")[-1].split("_")[-1].split(".")[0],alpha=0.5,c=colors[i],zorder=3)
